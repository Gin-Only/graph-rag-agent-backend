                                ...
                                if isinstance(token, str) and token.startswith("{") and token.endswith("}"):
                                    try:
                                        ...
                                        if "content" in json_data:
                                            ...
                                            if isinstance(token, str) and token.startswith("{") and token.endswith("}"):
                                                try:
                                                    nested_json = json.loads(token)
                                                    if "content" in nested_json:
                                                        token = nested_json["content"]
                                                except json.JSONDecodeError:
                                                    # 嵌套JSON解析失败，保持当前token
                                                    pass
                                            
                                            # 确保token不是空的JSON对象或无效的JSON字符串
                                            if token == "{}" or (isinstance(token, str) and token.startswith("{") and token.endswith("}") and len(token.strip()) <= 4):
                                                print(f"检测到可能的无效JSON内容: {token}")
                                                # 如果是空JSON或类似情况，使用空字符串
                                                token = ""
                        ...
                        with st.spinner("思考中..."):
                            try:
                                ...
                                if not full_response:
                                    print("流式响应为空，使用非流式API")
                                    response = send_message(prompt)
                                    if response:
                                        full_response = response.get("answer", "")
                                        message_placeholder.markdown(full_response)
                                # 检查响应是否仍然是JSON格式（可能表示解析失败）
                                elif full_response.strip().startswith("{") and full_response.strip().endswith("}") and "content" not in full_response:
                                    print("流式响应格式不正确，使用非流式API")
                                    print(f"当前响应内容: {full_response[:100]}")
                                    response = send_message(prompt)
                                    if response:
                                        full_response = response.get("answer", "")
                                        message_placeholder.markdown(full_response)
