...
def send_message_stream(message: str, on_token: Callable[[str, bool], None]) -> str:
    ...
    try:
        ...
        for event in client.events():
            print(f"[STREAM DEBUG] Raw SSE event: id={event.id}, event={event.event}, data={event.data[:500]}") # 添加日志记录
            try:
                ...
                if data.get("status") == "token":
                    ...
                    # 检查是否是空JSON对象或无效JSON
                    if content == "{}" or (isinstance(content, str) and content.startswith("{") and content.endswith("}") and len(content.strip()) <= 4):
                        print(f"API层检测到无效JSON内容: {content}")
                        content = ""
                    ...
                    formatted_error = f"\n\n⚠️ 错误: {error_message}"
                    # 如果是模型不可用错误，添加提示信息
                    if "模型暂时不可用" in error_message or "无可用渠道" in error_message:
                        formatted_error += "\n\n系统已自动切换到备用模型，请重新发送您的问题。"
                    on_token(formatted_error)
                    break
                else:
                    # 其他状态类型处理
                    pass
...
def send_feedback(message_id: str, query: str, is_positive: bool, thread_id: str, agent_type: str = "retrieval_agent"):
    """向后端发送用户反馈"""
    start_time = time.time()
    try:
        # 确保 agent_type 有值
