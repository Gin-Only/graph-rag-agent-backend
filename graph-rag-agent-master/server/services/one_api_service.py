...
logger = logging.getLogger("one_api_service")
...
class OneAPIService:
    ...
    def __init__(self):
        ...
        self.current_models = {
            ...
            "embeddings": os.getenv('OPENAI_EMBEDDINGS_MODEL', 'text-embedding-3-large')
        }
    ...
    def handle_api_error(self, error: Exception, model_type: str = "llm") -> Dict[str, Any]:
        ...
        if self._is_model_unavailable_error(error_info, current_model):
            ...
            if next_model:
                ...
                # 更新环境变量
                if model_type == "llm":
                    os.environ['OPENAI_LLM_MODEL'] = next_model
                else:
                    os.environ['OPENAI_EMBEDDINGS_MODEL'] = next_model
