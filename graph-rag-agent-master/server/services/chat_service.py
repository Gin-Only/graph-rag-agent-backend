...
async def process_chat(message: str, session_id: str, debug: bool = False, agent_type: str = "hybrid_agent", 
                       use_deeper_tool: bool = True, show_thinking: bool = False, evaluation_mode: bool = False) -> Dict:
    """
    处理聊天请求
    ...
    Args:
        ...
        evaluation_mode: 是否启用检索+评估模式 (for evaluator_agent)
    ...
    try:
        ...
        # 获取指定的agent
        try:
            # 检查是否为evaluator_agent模式（检索+评估模式）
            if agent_type == "evaluator_agent" and evaluation_mode:
                # 获取retrieval_agent和evaluator_agent
                retrieval_agent = agent_manager.get_agent("retrieval_agent")
                evaluator_agent = agent_manager.get_agent("evaluator_agent")
                selected_agent = evaluator_agent  # 主要使用evaluator_agent
            else:
                # 常规模式，获取单个agent
        ...
        if debug:
            ...
            if agent_type == "deep_research_agent" and show_thinking:
                ...
                # 检查是否为evaluator_agent模式（检索+评估模式）
                if agent_type == "evaluator_agent" and evaluation_mode:
                    # 先使用retrieval_agent获取答案
                    print("检索+评估模式: 使用retrieval_agent获取初始答案")
                    retrieval_answer = retrieval_agent.ask(message, thread_id=session_id)
                    
                    # 然后使用evaluator_agent评估答案
                    print("检索+评估模式: 使用evaluator_agent评估答案")
                    evaluation_prompt = f"请评估以下答案的质量和准确性，检测是否存在幻觉，并提供改进后的答案:\n\n问题: {message}\n\n答案: {retrieval_answer}"
                    
                    # 使用ask_with_trace获取完整的评估结果，包括高亮标注
                    evaluation_result = evaluator_agent.ask_with_trace(evaluation_prompt, thread_id=session_id)
                    final_answer = evaluation_result.get("answer", "")
                    ...
                    # 检查是否有高亮标注信息
                    if "execution_log" in evaluation_result:
                        for log in evaluation_result["execution_log"]:
                            if "output" in log and isinstance(log["output"], dict):
                                output_data = log["output"]
                                if "highlighted_answer" in output_data:
                                    highlighted_answer = output_data["highlighted_answer"]
                                if "has_hallucinations" in output_data:
                                    has_hallucinations = output_data["has_hallucinations"]
                                if "hallucination_count" in output_data:
                                    hallucination_count = output_data["hallucination_count"]
                    
                    # 构建响应，包含高亮标注信息
                    return {
                        "answer": final_answer,
                        "retrieval_answer": retrieval_answer,  # 保存原始检索答案以便前端展示
                        "evaluation_mode": True,  # 标记为评估模式
                        "highlighted_answer": highlighted_answer if highlighted_answer else final_answer,
...
async def process_chat_stream(
    ...
    """
    import json
    from server.services.one_api_service import one_api_service
    # 生成锁的键
    lock_key = f"{session_id}_chat"
    ...
    try:
        ...
        if agent_type in ["evaluator_agent", "retrieval_agent"]:
            # 检查是否为evaluator_agent的检索+评估模式
            if agent_type == "evaluator_agent" and evaluation_mode:
                # 先使用retrieval_agent获取答案
                print("检索+评估模式(流式): 使用retrieval_agent获取初始答案")
                retrieval_answer = retrieval_agent.ask(message, thread_id=session_id)
                
                # 发送检索答案的状态更新
                yield json.dumps({"status": "retrieval_complete", "retrieval_answer": retrieval_answer})
                
                # 然后使用evaluator_agent评估答案
                print("检索+评估模式(流式): 使用evaluator_agent评估答案")
                evaluation_prompt = f"请评估以下答案的质量和准确性，检测是否存在幻觉，并提供改进后的答案:\n\n问题: {message}\n\n答案: {retrieval_answer}"
                
                # 获取完整的评估结果，包括高亮标注
                evaluation_result = await asyncio.to_thread(
                    evaluator_agent.ask_with_trace,
                    evaluation_prompt,
                    thread_id=session_id
                )
        ...
        try:
            # 检查是否是API错误
            if "无可用渠道" in str(e) or "gpt-3.5-turbo" in str(e):
                # 使用One API服务处理错误
                error_result = one_api_service.handle_api_error(e, model_type="llm")
                
                # 如果成功切换到备用模型
                if error_result['status'] == 'fallback':
                    yield json.dumps({"status": "error", "message": f"模型暂时不可用，已自动切换到备用模型 {error_result['new_model']}，请重试"})
                else:
                    # 返回友好的错误消息
                    yield json.dumps({"status": "error", "message": error_result['message']})
            else:
                # 其他类型的错误
                yield json.dumps({"status": "error", "message": f"处理请求时出错: {str(e)}"})
        except Exception as handler_error:
            # 错误处理器本身出错
            print(f"错误处理失败: {str(handler_error)}")
            yield json.dumps({"status": "error", "message": str(e)})
    finally:
        # 释放锁
        chat_manager.release_lock(lock_key)
