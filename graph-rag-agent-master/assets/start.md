...
```env
# 模型提供商配置
MODEL_PROVIDER=alibaba  # 可选值: openai, ollama, alibaba

# One-API 配置（当 MODEL_PROVIDER=openai 时使用）
OPENAI_API_KEY='你的 one-api 令牌'
OPENAI_BASE_URL='http://localhost:13000/v1'
OPENAI_EMBEDDINGS_MODEL='嵌入模型名称'
OPENAI_LLM_MODEL='LLM 模型名称'

# 阿里巴巴API配置（当 MODEL_PROVIDER=alibaba 时使用）
ALIBABA_API_KEY='你的阿里巴巴API密钥'
ALIBABA_BASE_URL='你的API接口地址'  # 例如: /api/alibaba
ALIBABA_EMBEDDINGS_MODEL='text-embedding-v1'  # 可选值: text-embedding-v1, text-embedding-v2
ALIBABA_LLM_MODEL='qwen-turbo'  # 可选值: qwen-turbo, qwen-plus, qwen-max

# Ollama配置（当 MODEL_PROVIDER=ollama 时使用）
